...
specify sequences, use the sqlalchemy.schemas.Sequence object which is passed
...
schemas level communication, such as reflection of tables and indexes.   Using
...
class OracleDialect(default.DefaultDialect):
        ...
        if desired_owner:
            ...
            if len(rows) > 1:
                raise AssertionError(
                    "There are multiple tables visible to the schemas, you "
                    "must specify owner")
            elif len(rows) == 1:
                row = rows[0]
                return (row['table_name'], row['table_owner'],
    ...
    def get_indexes(self, connection, table_name, schema=None,
        ...
        if schema is not None:
            params['schemas'] = schema
            text += "AND a.table_owner = :schemas "
