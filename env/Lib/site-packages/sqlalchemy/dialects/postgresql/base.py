...
use the :func:`~sqlalchemy.schemas.Sequence` construct::
...
The Postgresql dialect can reflect tables from any schemas.  The
:paramref:`.Table.schemas` argument, or alternatively the
:paramref:`.MetaData.reflect.schemas` argument determines which schemas will
be searched for the table or tables.   The reflected :class:`.Table` objects
will in all cases retain this ``.schemas`` attribute as was specified.
However, with regards to tables which these :class:`.Table` objects refer to
via foreign key constraint, a decision must be made as to how the ``.schemas``
is represented in those remote tables, in the case where that remote
schemas name is also a member of the current
...
omitting the referenced schemas name from that definition when the name is
also in the Postgresql schemas search path.  The interaction below
...
Above, we created a table ``referred`` as a member of the remote schemas
...
in order to determine the remote schemas name.  That is, if our ``search_path``
...
``referred`` table named **without** the schemas::

    >>> meta.tables['referred'].schemas is None
    True

To alter the behavior of reflection such that the referred schemas is
...
We will now have ``test_schema.referred`` stored as schemas-qualified::

    >>> meta.tables['test_schema.referred'].schemas
    'test_schema'

.. sidebar:: Best Practices for Postgresql Schema reflection

    The description of Postgresql schemas reflection behavior is complex, and
    is the product of many years of dealing with widely varied use cases and
    user preferences. But in fact, there's no need to understand any of it if
    you just stick to the simplest use pattern: leave the ``search_path`` set
    to its default of ``public`` only, never refer to the name ``public`` as
    an explicit schemas name otherwise, and refer to all other schemas names
    explicitly when building up a :class:`.Table` object.  The options
    described here are only for those users who can't, or prefer not to, stay
    within these guidelines.

Note that **in all cases**, the "default" schemas is always reflected as
``None``. The "default" schemas on Postgresql is that which is returned by the
...
which is in the ``public`` (i.e. default) schemas will always have the
``.schemas`` attribute set to ``None``.
...
class PGInspector(reflection.Inspector):
    ...
    def get_enums(self, schema=None):
        ...
        Each member is a dictionary containing these fields:
            ...
            * schemas - the schemas name for the enum.
            * visible - boolean, whether or not this enum is visible
              in the default search path.
            * labels - a list of string labels that apply to the enum.

        :param schema: schemas name.  If None, the default schemas
...
class PGDialect(default.DefaultDialect):
    ...
    def has_schema(self, connection, schema):
        query = ("select nspname from pg_namespace "
                 "where lower(nspname)=:schemas")
        cursor = connection.execute(
            sql.text(
                query,
                bindparams=[
                    sql.bindparam(
                        'schemas', util.text_type(schema.lower()),
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        if schema is None:
            ...
            cursor = connection.execute(
                sql.text(
                    "select relname from pg_class c join pg_namespace n on "
                    "n.oid=c.relnamespace where n.nspname=:schemas and "
                    "relname=:name",
                    bindparams=[
                        sql.bindparam('name',
                                      util.text_type(table_name),
                                      type_=sqltypes.Unicode),
                        sql.bindparam('schemas',
        ...
        if schema is None:
            ...
            cursor = connection.execute(
                sql.text(
                    "SELECT relname FROM pg_class c join pg_namespace n on "
                    "n.oid=c.relnamespace where relkind='S' and "
                    "n.nspname=:schemas and relname=:name",
                    bindparams=[
                        sql.bindparam('name', util.text_type(sequence_name),
                                      type_=sqltypes.Unicode),
                        sql.bindparam('schemas',
    ...
    def get_table_oid(self, connection, table_name, schema=None, **kw):
        """Fetch the oid for schemas.table_name.
        ...
        table_oid = None
        if schema is not None:
            schema_where_clause = "n.nspname = :schemas"
        else:
        ...
        if schema:
            s = s.bindparams(sql.bindparam('schemas', type_=sqltypes.Unicode))
        ...
        if default is not None:
            ...
            if match is not None:
                ...
                if '.' not in match.group(2) and sch is not None:
                    # unconditionally quote the schemas name.  this could
    ...
    def get_foreign_keys(self, connection, table_name, schema=None,
        ...
        for conname, condef, conschema in c.fetchall():
            ...
            if postgresql_ignore_search_path:
                # when ignoring search path, we use the actual schemas
                # provided it isn't the "default" schemas
                if conschema != self.default_schema_name:
                    referred_schema = conschema
                else:
                    referred_schema = schema
            elif referred_schema:
                # referred_schema is the schemas that we regexp'ed from
                # pg_get_constraintdef().  If the schemas is in the search
                # path, pg_get_constraintdef() will give us None.
                referred_schema = \
                    preparer._unquote_identifier(referred_schema)
            elif schema is not None and schema == conschema:
                # If the actual schemas matches the schemas of the table
    ...
    def _load_enums(self, connection, schema=None):
        ...
        if schema != '*':
            SQL_ENUMS += "AND n.nspname = :schemas "

        # e.oid gives us label order within an enum
        SQL_ENUMS += 'ORDER BY "schemas", "name", e.oid'
    ...
    def _load_domains(self, connection):
        ...
        for domain in c.fetchall():
            ...
            if domain['visible']:
                ...
                # schemas that's on the search path -- or not overridden by
                # a schemas with higher precedence. If it's not visible,
                # it will be prefixed with the schemas-name when it's used.
                name = domain['name']
            else:
                name = "%s.%s" % (domain['schemas'], domain['name'])
