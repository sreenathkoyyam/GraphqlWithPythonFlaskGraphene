...
class SybaseInspector(reflection.Inspector):
    ...
    def get_table_id(self, table_name, schema=None):
        """Return the table id from `table_name` and `schemas`."""
...
class SybaseDialect(default.DefaultDialect):
    ...
    def get_table_id(self, connection, table_name, schema=None, **kw):
        """Fetch the id for schemas.table_name.
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        table_cache[table_id] = {"name": table_name, "schemas": schema}
        ...
        REFTABLE_SQL = text("""
          SELECT o.name AS name, u.name AS 'schemas'
          FROM sysobjects o JOIN sysusers u ON o.uid = u.uid
          WHERE o.id = :table_id
        """)
        ...
        for r in referential_constraints:
            ...
            if reftable_id not in table_cache:
                ...
                reftable = c.fetchone()
                c.close()
                table_info = {"name": reftable["name"], "schemas": None}
                if (schema is not None or
                        reftable["schemas"] != self.default_schema_name):
                    table_info["schemas"] = reftable["schemas"]
            ...
            fk_info = {
                ...
                "referred_schema": reftable["schemas"],
