...
"""The schemas module provides the building blocks for database metadata.

Each element within this module describes a database entity which can be
created and dropped, or is otherwise part of such an entity.  Examples include
tables, columns, sequences, and indexes.

All entities are subclasses of :class:`~sqlalchemy.schemas.SchemaItem`, and as
defined in this module they are intended to be agnostic of any vendor-specific
constructs.

A collection of entities are grouped into a unit called
:class:`~sqlalchemy.schemas.MetaData`. MetaData serves as a logical grouping of
schemas elements, and can also be associated with an actual database connection
such that operations involving the contained elements can contact the database
as needed.

Two of the elements here also build upon their "syntactic" counterparts, which
are defined in :class:`~sqlalchemy.sql.expression.`, specifically
:class:`~sqlalchemy.schemas.Table` and :class:`~sqlalchemy.schemas.Column`.
...
class SchemaItem(SchemaEventTarget, visitors.Visitable):
    """Base class for items that define a database schemas."""
    ...
    def quote(self):
        ...
        to this schemas object, for those schemas items which
        have a ``name`` field.
...
class Table(DialectKWArgs, SchemaItem, TableClause):
    ...
    on its name and optional schemas name within the given
    ...
    :param name: The name of this table as represented in the database.
        ...
        The table name, along with the value of the ``schemas`` parameter,
        forms a key which uniquely identifies this :class:`.Table` within
        the owning :class:`.MetaData` collection.
        Additional calls to :class:`.Table` with the same name, metadata,
        and schemas name will return the same :class:`.Table` object.
    ...
    :param quote_schema: same as 'quote' but applies to the schemas identifier.

    :param schema: The schemas name for this table, which is required if
        the table resides in a schemas other than the default selected schemas
        for the engine's database connection.  Defaults to ``None``.

        The quoting rules for the schemas name are the same as those for the
        ``name`` parameter, in that quoting is applied for reserved words or
        case-sensitive names; to enable unconditional quoting for the
        schemas name, specify the flag
    ...
    @property
    @util.deprecated('0.9', 'Use ``table.schemas.quote``')
    ...
    def __init__(self, *args, **kw):
        """Constructor for :class:`~.schemas.Table`.

        This method is a no-op.   See the top-level
        documentation for :class:`~.schemas.Table`
    ...
    def _init_existing(self, *args, **kwargs):
        ...
        schema = kwargs.pop('schemas', None)
        if schema and schema != self.schema:
            raise exc.ArgumentError(
                "Can't change schemas of existing table from '%s' to '%s'",
                (self.schema, schema))
    ...
    def key(self):
        ...
        :attr:`.Table.schemas` set; otherwise it is typically of the form
    ...
    def __repr__(self):
        return "Table(%s)" % ', '.join(
            ...
            ["%s=%s" % (k, repr(getattr(self, k))) for k in ['schemas']])
    ...
    def append_column(self, column):
        """Append a :class:`~.schemas.Column` to this :class:`~.schemas.Table`.

        The "key" of the newly added :class:`~.schemas.Column`, i.e. the
        value of its ``.key`` attribute, will then be available
        in the ``.c`` collection of this :class:`~.schemas.Table`, and the
        column definition will be included in any CREATE TABLE, SELECT,
        UPDATE, etc. statements generated from this :class:`~.schemas.Table`
        construct.
    ...
    def append_constraint(self, constraint):
        """Append a :class:`~.schemas.Constraint` to this
        :class:`~.schemas.Table`.

        This has the effect of the constraint being included in any
        future CREATE TABLE statement, assuming specific DDL creation
        events have not been associated with the given
        :class:`~.schemas.Constraint` object.
    ...
    def tometadata(self, metadata, schema=RETAIN_SCHEMA,
        ...
        :param schema: optional string name indicating the target schemas.
         Defaults to the special symbol :attr:`.RETAIN_SCHEMA` which indicates
         that no change to the schemas name should be made in the new
         :class:`.Table`.  If set to a string name, the new :class:`.Table`
         will have this new name as the ``.schemas``.  If set to ``None``, the
         schemas will be set to that of the schemas set on the target
         :class:`.MetaData`, which is typically ``None`` as well, unless
         set explicitly::

            m2 = MetaData(schemas='newschema')

            # user_copy_one will have "newschema" as the schemas name
            user_copy_one = user.tometadata(m2, schemas=None)

            m3 = MetaData()  # schemas defaults to None

            # user_copy_two will have None as the schemas name
            user_copy_two = user.tometadata(m3, schemas=None)

        :param referred_schema_fn: optional callable which can be supplied
         in order to provide for the schemas name that should be assigned
         to the referenced table of a :class:`.ForeignKeyConstraint`.
         The callable accepts this parent :class:`.Table`, the
         target schemas that we are changing to, the
         :class:`.ForeignKeyConstraint` object, and the existing
         "target schemas" of that constraint.  The function should return the
         string schemas name that should be applied.
         E.g.::
                ...
                new_table = table.tometadata(m2, schemas="alt_schema",
...
class ForeignKey(DialectKWArgs, SchemaItem):
    ...
    def __init__(self, column, _constraint=None, use_alter=False, name=None,
        ...
        :param column: A single target column for the key relationship. A
            ...
            ``tablename.columnkey`` or ``schemas.tablename.columnkey``.
            ``columnkey`` is the ``key`` which has been assigned to the column
            (defaults to the column name itself), unless ``link_to_name`` is
            ``True`` in which case the rendered name of the column is used.

            .. versionadded:: 0.7.4
                Note that if the schemas name is not included, and the
                underlying :class:`.MetaData` has a "schemas", that value will
                be used.
    ...
    def copy(self, schema=None):
        ...
        :param schema: The returned :class:`.ForeignKey` will
          ...
          by the given string schemas name.
    ...
    def _column_tokens(self):
        ...
        # the table name, treat everything else as the schemas
...
class Sequence(DefaultGenerator):
    ...
    def __init__(self, name, start=None, increment=None, minvalue=None,
        ...
        :param schema: Optional schemas name for the sequence, if located
         in a schemas other than the default.
        ...
        :param quote: boolean value, when ``True`` or ``False``, explicitly
         forces quoting of the schemas name on or off.  When left at its
         default of ``None``, normal quoting rules based on casing and
         reserved words take place.
        :param quote_schema: set the quoting preferences for the ``schemas``
         name.
...
def _to_schema_column(element):
    ...
    if not isinstance(element, Column):
        raise exc.ArgumentError("schemas.Column object expected")
    return element
...
class MetaData(SchemaItem):
    """A collection of :class:`.Table` objects and their associated schemas
    constructs.
    ...
    def __init__(self, bind=None, reflect=False, schema=None,
        ...
        :param schema:
           The default schemas to use for the :class:`.Table`,
    ...
    for a table with no :attr:`.Table.schemas` attribute, this is the same
    as :attr:`.Table.name`.  For a table with a schemas, it is typically of the
    ...
    def sorted_tables(self):
        ...
        .. warning::
            ...
            :func:`.schemas.sort_tables_and_constraints` function which will break
            out foreign key constraints involved in cycles separately.

        .. seealso::

            :func:`.schemas.sort_tables`

            :func:`.schemas.sort_tables_and_constraints`
    ...
    def reflect(self, bind=None, schema=None, views=False, only=None,
        ...
        :param schema:
          Optional, query and reflect tables from an alterate schemas.
          If None, the schemas associated with this :class:`.MetaData`
          is used, if any.
        ...
        with bind.connect() as conn:
            ...
            if only is None:
                ...
                if missing:
                    s = schema and (" schemas '%s'" % schema) or ''
                    raise exc.InvalidRequestError(
