...
class FromClause(Selectable):
    ...
    * Methods to generate various derivations of a "from" clause, including
    ...
    def corresponding_column(self, column, require_embedded=False):
        ...
        :class:`~sqlalchemy.schemas.Column` via a common ancestor
        column.
...
class TableClause(Immutable, FromClause):
    ...
    the more commonly used :class:`~.schemas.Table` object, providing
    the usual set of :class:`~.expression.FromClause` services including
    the ``.c.`` collection and statement generation methods.

    It does **not** provide all the additional schemas-level services
    of :class:`~.schemas.Table`, including constraints, references to other
    tables, or support for :class:`.MetaData`-level services.  It's useful
    on its own as an ad-hoc construct used to generate quick SQL
    statements when a more fully fledged :class:`~.schemas.Table`
    is not on hand.
    ...
    def __init__(self, name, *columns):
        ...
        represents the "syntactical" portion of the schemas-level
        :class:`~.schemas.Table` object.
...
class CompoundSelect(GenerativeSelect):
    ...
    def _populate_column_collection(self):
        for cols in zip(*[s.c._all_columns for s in self.selects]):
            ...
            # that are "shared" - schemas.column can then copy all the
