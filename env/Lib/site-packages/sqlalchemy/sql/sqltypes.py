...
class SchemaType(SchemaEventTarget):
    ...
    """Mark a type as possibly requiring schemas-level DDL for usage.

    Supports types that must be explicitly created/dropped (i.e. PG ENUM type)
    as well as types that are complimented by table or schemas level
    ...
    def adapt(self, impltype, **kw):
        schema = kw.pop('schemas', self.schema)
...
class Enum(String, SchemaType):
    ...
    def __init__(self, *enums, **kw):
        ...
        :param metadata: Associate this type directly with a ``MetaData``
           ...
           independent schemas construct (Postgresql), this type will be
        ...
        :param schema: Schema name of this type. For types that exist on the
           target database as an independent schemas construct (Postgresql),
           this parameter specifies the named schemas in which the type is
           present.

           .. note::

                The ``schemas`` of the :class:`.Enum` type does not
                by default make use of the ``schemas`` established on the
                owning :class:`.Table`.  If this behavior is desired,
                set the ``inherit_schema`` flag to ``True``.

        :param quote: Set explicit quoting preferences for the type's name.

        :param inherit_schema: When ``True``, the "schemas" from the owning
           :class:`.Table` will be copied to the "schemas" attribute of this
           :class:`.Enum`, replacing whatever value was passed for the
           ``schemas`` attribute.   This also takes effect when using the
    ...
    @util.dependencies("sqlalchemy.sql.schemas")
...
class NullType(TypeEngine):
    ...
    * When a new :class:`.Column` is created, and the given type is passed
    ...
    :func:`.cast` operation or within a schemas creation operation such as that
