...
schemas, typically though not necessarily one which is reflected.
...
asking it to reflect the schemas and produce mappings::
...
established based on the table name we use.  If our schemas contains tables
``user`` and ``address``, we can define one or both of the classes to be used::
    ...
    class User(Base):
        ...
        # override schemas elements like Columns
...
relationship names based on a schemas, which means it has decision points in how
...
The above schemas will first automap the ``table_a`` table as a class named
...
class AutomapBase(object):
    """Base class for an "automap" schemas.
    ...
    def prepare(
        ...
        :param engine: an :class:`.Engine` or :class:`.Connection` with which
         to perform schemas reflection, if specified.
