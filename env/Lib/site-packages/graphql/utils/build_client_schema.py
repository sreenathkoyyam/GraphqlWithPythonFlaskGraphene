...
def build_client_schema(introspection):
    ...
    def get_named_type(type_name):
        ...
        if not type_introspection:
            raise Exception(
                "Invalid or incomplete schemas, unknown type: {}. Ensure that a full introspection query "
                "is used in order to build a client schemas.".format(type_name)
            )
    ...
    def build_type(type):
        ...
        if not handler:
            raise Exception(
                "Invalid or incomplete schemas, unknown kind: {}. Ensure that a full introspection query "
                "is used in order to build a client schemas.".format(type_kind)
            )
