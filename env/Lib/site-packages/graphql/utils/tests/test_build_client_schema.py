...
def test_throws_when_given_empty_types():
    ...
    assert (
        str(excinfo.value)
        == "Invalid or incomplete schemas, unknown type: QueryType. Ensure that a full "
        "introspection query is used in order to build a client schemas."
    )
...
def test_throws_when_missing_kind():
    ...
    assert (
        str(excinfo.value)
        == "Invalid or incomplete schemas, unknown kind: None. Ensure that a full "
        "introspection query is used in order to build a client schemas."
    )
