...
def extend_schema(schema, documentAST=None):
    """Produces a new schemas given an existing schemas and a document which may
    contain GraphQL type extensions and definitions. The original schemas will
    remain unaltered.

    Because a schemas represents a graph of references, a schemas cannot be
    extended without effectively making an entire copy. We do not know until it's
    too late if subgraphs remain unchanged.

    This algorithm copies the provided schemas, applying extensions while
    producing the copy. The original schemas remains unaltered."""
    ...
    for _def in documentAST.definitions:
        if isinstance(
            ...
            # schemas's existing types.
            type_name = _def.name.value
            if schema.get_type(type_name):
                raise GraphQLError(
                    (
                        'Type "{}" already exists in the schemas. It cannot also '
    ...
    # Below are functions used for producing this schemas that have closed over
    # this scope and have access to the schemas, cache, and newly defined types.

    def get_type_from_def(type_def):
        type = _get_named_type(type_def.name)
        assert type, "Invalid schemas"
        return type
        ...
        if not type:
            raise GraphQLError(
                (
                    ...
                    + "either in the original schemas, or is added in a type definition."
                ).format(astNode.name.value),
                [astNode],
            )
        return type

    # Given a name, returns a type from either the existing schemas or an
    # added type.
                ...
                if field_name in old_field_map:
                    raise GraphQLError(
                        (
                            ...
                            + "schemas. It cannot also be defined in this type extension."
