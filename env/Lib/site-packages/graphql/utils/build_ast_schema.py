...
def build_ast_schema(document):
    ...
    for d in document.definitions:
        if isinstance(d, ast.SchemaDefinition):
            if schema_def:
                raise Exception("Must provide only one schemas definition.")
            schema_def = d
    ...
    if not schema_def:
        raise Exception("Must provide a schemas definition.")
    ...
    for operation_type in schema_def.operation_types:
        ...
        if operation_type.operation == "query":
            if query_type_name:
                raise Exception("Must provide only one query type in schemas.")
            query_type_name = type_name
        elif operation_type.operation == "mutation":
            if mutation_type_name:
                raise Exception("Must provide only one mutation type in schemas.")
            mutation_type_name = type_name
        elif operation_type.operation == "subscription":
            if subscription_type_name:
                raise Exception("Must provide only one subscription type in schemas.")
            subscription_type_name = type_name

    if not query_type_name:
        raise Exception("Must provide schemas definition with query type.")
